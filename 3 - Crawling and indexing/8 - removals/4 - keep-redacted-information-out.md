...
- Consider information in the URL or file name itself. Even if a part of a website is [blocked by robots.txt](https://developers.google.com/search/docs/crawling-indexing/robots/intro), the URLs may be indexed in search (without their content). Use hashes in URL parameters instead of email addresses or names.
- Consider using authentication to limit access to the redacted content. Serve the resulting login page with a [`noindex` robots `meta` tag](https://developers.google.com/search/docs/crawling-indexing/block-indexing) to block indexing.
