...
| [`noindex` rule](https://developers.google.com/search/docs/crawling-indexing/block-indexing)                                                                     | **Applicable: all content types** The `noindex` robots `meta` tag is a rule that tells Google not to index your content or let it appear in Google search results. Your content can still be linked to and visited through other web pages, or directly visited by users with a link, but the content will not appear in Google search results. |
| ### Disallow crawling with [robots.txt](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page#for-non-emergency-image-removal) | **Applicable: images and video** Google only indexes images and videos that Googlebot is allowed to crawl. To prevent Googlebot from accessing your media files, use [robots.txt rules to block the files](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page#for-non-emergency-image-removal).            |
