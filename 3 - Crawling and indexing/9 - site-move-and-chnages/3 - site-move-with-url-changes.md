...
1. [**General best practices for site moves**](https://developers.google.com/search/docs/crawling-indexing/site-move-with-url-changes#general_recommendations_for_site_moves). Know what to expect, and how it might affect your users and rankings. If moving from HTTP to HTTPS, review the [best practices for HTTPS](https://web.dev/articles/enable-https).
...
- **Start with your important URLs** . To find them:
  - Look in your [sitemaps](https://developers.google.com/search/docs/crawling-indexing/sitemaps/overview) because it's likely your most important URLs have been submitted in Search Console that way
...
1. **Update annotations** to point to the new URLs in the HTML or sitemaps entry for each page:
   ...
   2. If the site you moved has multilingual or multinational pages annotated using [`rel-alternate-hreflang` annotations](https://developers.google.com/search/docs/specialty/international/localized-versions), be sure to update the annotations to use the new URLs.
...
- **Use server side permanent redirects** if technically possible. Although Googlebot supports [several kinds of redirects](https://developers.google.com/search/docs/crawling-indexing/301-redirects), we recommend that you use HTTP permanent redirects if possible, such as `301` and `308`.
...
2. **Check the `rel="canonical"` `link` annotations and
   `robots` `meta` rules:** Once the redirects are active, ensure that the `rel="canonical"` `link` annotations on the new site are using the new URLs. Similarly, if you added `noindex` `robots` `meta` rules to the new site to avoid prematurely indexing the new URLs, make sure to update them.
...
4. **If you're changing domain names or subdomains, submit a
   [Change of Address](https://support.google.com/webmasters/answer/9370220)
   in Search Console for the old site** .

   You only need this tool when moving from one domain or subdomain to another (such as
   `example.com` to `example.net`, or `a.example.com` to
   `b.example.com`). You don't need it for HTTP to
   HTTPS moves, switching between www and non-www on the same domain, or moving paths
