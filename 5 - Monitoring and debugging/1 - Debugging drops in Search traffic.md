...
[Video](https://www.youtube.com/watch?v=IIvIRgpCOqc)
...
Sketches showing traffic drop graphs and their potential causes Large drop from an algorithmic update, site-wide security or spam issue
Seasonality
...
A _small drop in position_ is when there's a small shift in position in the top results
...
A _large drop in position_ is when you see a notable drop out of the top results for a wide
...
If both impressions and clicks dropped, check the [list of the most common reasons](https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops#main-causes) that
...
([check the data](https://trends.google.com/trends/explore?date=today+5-y&geo=US&q=turkey,chicken,coffee)):
...
- **Check top queries in your region and compare them to the queries that you're getting
  traffic from**, as shown in Search Console's Performance report. If there are queries missing from your traffic, check if you have content on that subject and make sure it's being crawled and indexed.
